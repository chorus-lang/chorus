...
fn main() {
    ...
    match matches.value_of("lang").unwrap() {
        "ts" | "typescript" => compile(String::from(matches.value_of("INPUT_FILE").unwrap()), String::from(matches.value_of("lang").unwrap())),
        _ => println!("{} '{}' is not a valid compile target.", "Error:".red().bold(), matches.value_of("lang").unwrap().yellow())
    }
}
...
fn compile(input_file: String, lang: String) {
    ...
    if !read_dir(env::current_dir().unwrap()).unwrap().any(|file| file.unwrap().file_name() == std::ffi::OsString::from(&input_file)) {
        println!("{} Unable to find that file in this directory.", "Error:".red().bold());
        std::process::exit(0);
    }
    ...
    loop {
        ...
        if *done_clone.lock().unwrap() { break } else { continue };
    }
